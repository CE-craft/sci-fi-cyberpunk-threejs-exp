/*
Auto-generated by: https://github.com/pmndrs/gltfjsx
*/

import React, { useRef } from "react";
import { useGLTF } from "@react-three/drei";
import { useFrame } from "@react-three/fiber";
import { useEffect } from "react";

export default function Model({ ...props }) {
  const group = useRef();

  useFrame(({ clock }) => {
    group.current.rotation.y = clock.elapsedTime * Math.PI * 2 * 0.2;
  });
  const { nodes, materials } = useGLTF("/cyberpunk-robot.glb");

  return (
    <group ref={group} {...props} dispose={null}>
      <mesh
        geometry={nodes.Cylinder106.geometry}
        material={nodes.Cylinder106.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve047.geometry}
        material={nodes.Curve047.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve048.geometry}
        material={nodes.Curve048.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder104.geometry}
        material={nodes.Cylinder104.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder105.geometry}
        material={nodes.Cylinder105.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube034.geometry}
        material={nodes.Tube034.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube035.geometry}
        material={nodes.Tube035.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube036.geometry}
        material={nodes.Tube036.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube037.geometry}
        material={nodes.Tube037.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane162.geometry}
        material={nodes.Plane162.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box027.geometry}
        material={nodes.Box027.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box028.geometry}
        material={nodes.Box028.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube033.geometry}
        material={nodes.Tube033.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder103.geometry}
        material={nodes.Cylinder103.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object157.geometry}
        material={nodes.Object157.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box024.geometry}
        material={nodes.Box024.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box025.geometry}
        material={nodes.Box025.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box026.geometry}
        material={nodes.Box026.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object142.geometry}
        material={nodes.Object142.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane148.geometry}
        material={nodes.Plane148.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object143.geometry}
        material={nodes.Object143.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane149.geometry}
        material={nodes.Plane149.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object144.geometry}
        material={nodes.Object144.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane150.geometry}
        material={nodes.Plane150.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object145.geometry}
        material={nodes.Object145.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane151.geometry}
        material={nodes.Plane151.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object146.geometry}
        material={nodes.Object146.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane152.geometry}
        material={nodes.Plane152.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object147.geometry}
        material={nodes.Object147.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane153.geometry}
        material={nodes.Plane153.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object148.geometry}
        material={nodes.Object148.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane154.geometry}
        material={nodes.Plane154.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object149.geometry}
        material={nodes.Object149.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane155.geometry}
        material={nodes.Plane155.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object150.geometry}
        material={nodes.Object150.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane156.geometry}
        material={nodes.Plane156.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object151.geometry}
        material={nodes.Object151.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane157.geometry}
        material={nodes.Plane157.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object152.geometry}
        material={nodes.Object152.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane158.geometry}
        material={nodes.Plane158.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object153.geometry}
        material={nodes.Object153.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane159.geometry}
        material={nodes.Plane159.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box029.geometry}
        material={nodes.Box029.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder107.geometry}
        material={nodes.Cylinder107.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder108.geometry}
        material={nodes.Cylinder108.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder109.geometry}
        material={nodes.Cylinder109.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder110.geometry}
        material={nodes.Cylinder110.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder111.geometry}
        material={nodes.Cylinder111.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder112.geometry}
        material={nodes.Cylinder112.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder113.geometry}
        material={nodes.Cylinder113.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder114.geometry}
        material={nodes.Cylinder114.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder115.geometry}
        material={nodes.Cylinder115.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder116.geometry}
        material={nodes.Cylinder116.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder117.geometry}
        material={nodes.Cylinder117.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder118.geometry}
        material={nodes.Cylinder118.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder119.geometry}
        material={nodes.Cylinder119.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder120.geometry}
        material={nodes.Cylinder120.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder121.geometry}
        material={nodes.Cylinder121.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder122.geometry}
        material={nodes.Cylinder122.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder123.geometry}
        material={nodes.Cylinder123.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder124.geometry}
        material={nodes.Cylinder124.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder125.geometry}
        material={nodes.Cylinder125.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder126.geometry}
        material={nodes.Cylinder126.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder127.geometry}
        material={nodes.Cylinder127.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder128.geometry}
        material={nodes.Cylinder128.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder129.geometry}
        material={nodes.Cylinder129.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder130.geometry}
        material={nodes.Cylinder130.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder131.geometry}
        material={nodes.Cylinder131.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder132.geometry}
        material={nodes.Cylinder132.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Lag_006.geometry}
        material={nodes.Lag_006.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object154.geometry}
        material={nodes.Object154.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object155.geometry}
        material={nodes.Object155.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object156.geometry}
        material={nodes.Object156.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane160.geometry}
        material={nodes.Plane160.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane161.geometry}
        material={nodes.Plane161.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube032.geometry}
        material={nodes.Tube032.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve037.geometry}
        material={nodes.Curve037.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve038.geometry}
        material={nodes.Curve038.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve039.geometry}
        material={nodes.Curve039.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve040.geometry}
        material={nodes.Curve040.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve041.geometry}
        material={nodes.Curve041.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve042.geometry}
        material={nodes.Curve042.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve043.geometry}
        material={nodes.Curve043.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve044.geometry}
        material={nodes.Curve044.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve045.geometry}
        material={nodes.Curve045.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve046.geometry}
        material={nodes.Curve046.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane163.geometry}
        material={nodes.Plane163.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane164.geometry}
        material={nodes.Plane164.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object158.geometry}
        material={nodes.Object158.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane180.geometry}
        material={nodes.Plane180.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box061.geometry}
        material={nodes.Box061.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box062.geometry}
        material={nodes.Box062.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube039.geometry}
        material={nodes.Tube039.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder133.geometry}
        material={nodes.Cylinder133.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object232.geometry}
        material={nodes.Object232.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box058.geometry}
        material={nodes.Box058.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box059.geometry}
        material={nodes.Box059.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box060.geometry}
        material={nodes.Box060.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object217.geometry}
        material={nodes.Object217.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane166.geometry}
        material={nodes.Plane166.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object218.geometry}
        material={nodes.Object218.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane167.geometry}
        material={nodes.Plane167.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object219.geometry}
        material={nodes.Object219.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane168.geometry}
        material={nodes.Plane168.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object220.geometry}
        material={nodes.Object220.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane169.geometry}
        material={nodes.Plane169.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object221.geometry}
        material={nodes.Object221.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane170.geometry}
        material={nodes.Plane170.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object222.geometry}
        material={nodes.Object222.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane171.geometry}
        material={nodes.Plane171.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object223.geometry}
        material={nodes.Object223.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane172.geometry}
        material={nodes.Plane172.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object224.geometry}
        material={nodes.Object224.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane173.geometry}
        material={nodes.Plane173.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object225.geometry}
        material={nodes.Object225.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane174.geometry}
        material={nodes.Plane174.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object226.geometry}
        material={nodes.Object226.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane175.geometry}
        material={nodes.Plane175.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object227.geometry}
        material={nodes.Object227.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane176.geometry}
        material={nodes.Plane176.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object228.geometry}
        material={nodes.Object228.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane177.geometry}
        material={nodes.Plane177.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box063.geometry}
        material={nodes.Box063.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder137.geometry}
        material={nodes.Cylinder137.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder138.geometry}
        material={nodes.Cylinder138.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder139.geometry}
        material={nodes.Cylinder139.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder140.geometry}
        material={nodes.Cylinder140.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder141.geometry}
        material={nodes.Cylinder141.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder142.geometry}
        material={nodes.Cylinder142.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder143.geometry}
        material={nodes.Cylinder143.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder144.geometry}
        material={nodes.Cylinder144.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder145.geometry}
        material={nodes.Cylinder145.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder146.geometry}
        material={nodes.Cylinder146.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder147.geometry}
        material={nodes.Cylinder147.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder148.geometry}
        material={nodes.Cylinder148.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder149.geometry}
        material={nodes.Cylinder149.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder150.geometry}
        material={nodes.Cylinder150.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder151.geometry}
        material={nodes.Cylinder151.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder152.geometry}
        material={nodes.Cylinder152.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder153.geometry}
        material={nodes.Cylinder153.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder154.geometry}
        material={nodes.Cylinder154.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder155.geometry}
        material={nodes.Cylinder155.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder156.geometry}
        material={nodes.Cylinder156.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder157.geometry}
        material={nodes.Cylinder157.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder158.geometry}
        material={nodes.Cylinder158.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder159.geometry}
        material={nodes.Cylinder159.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder160.geometry}
        material={nodes.Cylinder160.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder161.geometry}
        material={nodes.Cylinder161.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder162.geometry}
        material={nodes.Cylinder162.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Lag_007.geometry}
        material={nodes.Lag_007.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object229.geometry}
        material={nodes.Object229.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object230.geometry}
        material={nodes.Object230.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object231.geometry}
        material={nodes.Object231.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane178.geometry}
        material={nodes.Plane178.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane179.geometry}
        material={nodes.Plane179.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube038.geometry}
        material={nodes.Tube038.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve049.geometry}
        material={nodes.Curve049.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve050.geometry}
        material={nodes.Curve050.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve051.geometry}
        material={nodes.Curve051.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve052.geometry}
        material={nodes.Curve052.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve053.geometry}
        material={nodes.Curve053.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve054.geometry}
        material={nodes.Curve054.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve055.geometry}
        material={nodes.Curve055.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve056.geometry}
        material={nodes.Curve056.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve057.geometry}
        material={nodes.Curve057.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve058.geometry}
        material={nodes.Curve058.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane181.geometry}
        material={nodes.Plane181.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane182.geometry}
        material={nodes.Plane182.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object233.geometry}
        material={nodes.Object233.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder136.geometry}
        material={nodes.Cylinder136.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve059.geometry}
        material={nodes.Curve059.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve060.geometry}
        material={nodes.Curve060.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder134.geometry}
        material={nodes.Cylinder134.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder135.geometry}
        material={nodes.Cylinder135.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube040.geometry}
        material={nodes.Tube040.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube041.geometry}
        material={nodes.Tube041.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube042.geometry}
        material={nodes.Tube042.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube043.geometry}
        material={nodes.Tube043.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane197.geometry}
        material={nodes.Plane197.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box067.geometry}
        material={nodes.Box067.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box068.geometry}
        material={nodes.Box068.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube045.geometry}
        material={nodes.Tube045.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder163.geometry}
        material={nodes.Cylinder163.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object249.geometry}
        material={nodes.Object249.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box064.geometry}
        material={nodes.Box064.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box065.geometry}
        material={nodes.Box065.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box066.geometry}
        material={nodes.Box066.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object234.geometry}
        material={nodes.Object234.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane183.geometry}
        material={nodes.Plane183.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object235.geometry}
        material={nodes.Object235.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane184.geometry}
        material={nodes.Plane184.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object236.geometry}
        material={nodes.Object236.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane185.geometry}
        material={nodes.Plane185.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object237.geometry}
        material={nodes.Object237.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane186.geometry}
        material={nodes.Plane186.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object238.geometry}
        material={nodes.Object238.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane187.geometry}
        material={nodes.Plane187.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object239.geometry}
        material={nodes.Object239.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane188.geometry}
        material={nodes.Plane188.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object240.geometry}
        material={nodes.Object240.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane189.geometry}
        material={nodes.Plane189.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object241.geometry}
        material={nodes.Object241.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane190.geometry}
        material={nodes.Plane190.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object242.geometry}
        material={nodes.Object242.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane191.geometry}
        material={nodes.Plane191.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object243.geometry}
        material={nodes.Object243.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane192.geometry}
        material={nodes.Plane192.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object244.geometry}
        material={nodes.Object244.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane193.geometry}
        material={nodes.Plane193.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object245.geometry}
        material={nodes.Object245.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane194.geometry}
        material={nodes.Plane194.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box069.geometry}
        material={nodes.Box069.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder167.geometry}
        material={nodes.Cylinder167.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder168.geometry}
        material={nodes.Cylinder168.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder169.geometry}
        material={nodes.Cylinder169.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder170.geometry}
        material={nodes.Cylinder170.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder171.geometry}
        material={nodes.Cylinder171.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder172.geometry}
        material={nodes.Cylinder172.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder173.geometry}
        material={nodes.Cylinder173.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder174.geometry}
        material={nodes.Cylinder174.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder175.geometry}
        material={nodes.Cylinder175.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder176.geometry}
        material={nodes.Cylinder176.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder177.geometry}
        material={nodes.Cylinder177.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder178.geometry}
        material={nodes.Cylinder178.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder179.geometry}
        material={nodes.Cylinder179.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder180.geometry}
        material={nodes.Cylinder180.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder181.geometry}
        material={nodes.Cylinder181.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder182.geometry}
        material={nodes.Cylinder182.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder183.geometry}
        material={nodes.Cylinder183.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder184.geometry}
        material={nodes.Cylinder184.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder185.geometry}
        material={nodes.Cylinder185.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder186.geometry}
        material={nodes.Cylinder186.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder187.geometry}
        material={nodes.Cylinder187.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder188.geometry}
        material={nodes.Cylinder188.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder189.geometry}
        material={nodes.Cylinder189.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder190.geometry}
        material={nodes.Cylinder190.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder191.geometry}
        material={nodes.Cylinder191.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder192.geometry}
        material={nodes.Cylinder192.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Lag_008.geometry}
        material={nodes.Lag_008.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object246.geometry}
        material={nodes.Object246.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object247.geometry}
        material={nodes.Object247.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object248.geometry}
        material={nodes.Object248.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane195.geometry}
        material={nodes.Plane195.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane196.geometry}
        material={nodes.Plane196.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube044.geometry}
        material={nodes.Tube044.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve061.geometry}
        material={nodes.Curve061.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve062.geometry}
        material={nodes.Curve062.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve063.geometry}
        material={nodes.Curve063.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve064.geometry}
        material={nodes.Curve064.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve065.geometry}
        material={nodes.Curve065.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve066.geometry}
        material={nodes.Curve066.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve067.geometry}
        material={nodes.Curve067.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve068.geometry}
        material={nodes.Curve068.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve069.geometry}
        material={nodes.Curve069.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve070.geometry}
        material={nodes.Curve070.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane198.geometry}
        material={nodes.Plane198.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane199.geometry}
        material={nodes.Plane199.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object250.geometry}
        material={nodes.Object250.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder166.geometry}
        material={nodes.Cylinder166.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve071.geometry}
        material={nodes.Curve071.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve072.geometry}
        material={nodes.Curve072.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder164.geometry}
        material={nodes.Cylinder164.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder165.geometry}
        material={nodes.Cylinder165.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube046.geometry}
        material={nodes.Tube046.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube047.geometry}
        material={nodes.Tube047.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube048.geometry}
        material={nodes.Tube048.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube049.geometry}
        material={nodes.Tube049.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane248.geometry}
        material={nodes.Plane248.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box085.geometry}
        material={nodes.Box085.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box086.geometry}
        material={nodes.Box086.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube063.geometry}
        material={nodes.Tube063.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder253.geometry}
        material={nodes.Cylinder253.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object300.geometry}
        material={nodes.Object300.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box082.geometry}
        material={nodes.Box082.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box083.geometry}
        material={nodes.Box083.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box084.geometry}
        material={nodes.Box084.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object285.geometry}
        material={nodes.Object285.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane234.geometry}
        material={nodes.Plane234.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object286.geometry}
        material={nodes.Object286.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane235.geometry}
        material={nodes.Plane235.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object287.geometry}
        material={nodes.Object287.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane236.geometry}
        material={nodes.Plane236.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object288.geometry}
        material={nodes.Object288.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane237.geometry}
        material={nodes.Plane237.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object289.geometry}
        material={nodes.Object289.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane238.geometry}
        material={nodes.Plane238.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object290.geometry}
        material={nodes.Object290.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane239.geometry}
        material={nodes.Plane239.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object291.geometry}
        material={nodes.Object291.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane240.geometry}
        material={nodes.Plane240.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object292.geometry}
        material={nodes.Object292.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane241.geometry}
        material={nodes.Plane241.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object293.geometry}
        material={nodes.Object293.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane242.geometry}
        material={nodes.Plane242.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object294.geometry}
        material={nodes.Object294.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane243.geometry}
        material={nodes.Plane243.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object295.geometry}
        material={nodes.Object295.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane244.geometry}
        material={nodes.Plane244.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object296.geometry}
        material={nodes.Object296.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane245.geometry}
        material={nodes.Plane245.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box087.geometry}
        material={nodes.Box087.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder257.geometry}
        material={nodes.Cylinder257.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder258.geometry}
        material={nodes.Cylinder258.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder259.geometry}
        material={nodes.Cylinder259.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder260.geometry}
        material={nodes.Cylinder260.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder261.geometry}
        material={nodes.Cylinder261.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder262.geometry}
        material={nodes.Cylinder262.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder263.geometry}
        material={nodes.Cylinder263.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder264.geometry}
        material={nodes.Cylinder264.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder265.geometry}
        material={nodes.Cylinder265.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder266.geometry}
        material={nodes.Cylinder266.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder267.geometry}
        material={nodes.Cylinder267.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder268.geometry}
        material={nodes.Cylinder268.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder269.geometry}
        material={nodes.Cylinder269.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder270.geometry}
        material={nodes.Cylinder270.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder271.geometry}
        material={nodes.Cylinder271.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder272.geometry}
        material={nodes.Cylinder272.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder273.geometry}
        material={nodes.Cylinder273.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder274.geometry}
        material={nodes.Cylinder274.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder275.geometry}
        material={nodes.Cylinder275.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder276.geometry}
        material={nodes.Cylinder276.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder277.geometry}
        material={nodes.Cylinder277.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder278.geometry}
        material={nodes.Cylinder278.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder279.geometry}
        material={nodes.Cylinder279.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder280.geometry}
        material={nodes.Cylinder280.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder281.geometry}
        material={nodes.Cylinder281.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder282.geometry}
        material={nodes.Cylinder282.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Lag_011.geometry}
        material={nodes.Lag_011.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object297.geometry}
        material={nodes.Object297.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object298.geometry}
        material={nodes.Object298.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object299.geometry}
        material={nodes.Object299.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane246.geometry}
        material={nodes.Plane246.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane247.geometry}
        material={nodes.Plane247.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube062.geometry}
        material={nodes.Tube062.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve097.geometry}
        material={nodes.Curve097.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve098.geometry}
        material={nodes.Curve098.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve099.geometry}
        material={nodes.Curve099.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve100.geometry}
        material={nodes.Curve100.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve101.geometry}
        material={nodes.Curve101.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve102.geometry}
        material={nodes.Curve102.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve103.geometry}
        material={nodes.Curve103.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve104.geometry}
        material={nodes.Curve104.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve105.geometry}
        material={nodes.Curve105.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve106.geometry}
        material={nodes.Curve106.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane249.geometry}
        material={nodes.Plane249.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane250.geometry}
        material={nodes.Plane250.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object301.geometry}
        material={nodes.Object301.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder256.geometry}
        material={nodes.Cylinder256.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve107.geometry}
        material={nodes.Curve107.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve108.geometry}
        material={nodes.Curve108.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder254.geometry}
        material={nodes.Cylinder254.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder255.geometry}
        material={nodes.Cylinder255.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube064.geometry}
        material={nodes.Tube064.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube065.geometry}
        material={nodes.Tube065.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube066.geometry}
        material={nodes.Tube066.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube067.geometry}
        material={nodes.Tube067.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane231.geometry}
        material={nodes.Plane231.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box079.geometry}
        material={nodes.Box079.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box080.geometry}
        material={nodes.Box080.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube057.geometry}
        material={nodes.Tube057.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder223.geometry}
        material={nodes.Cylinder223.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object283.geometry}
        material={nodes.Object283.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box076.geometry}
        material={nodes.Box076.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box077.geometry}
        material={nodes.Box077.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box078.geometry}
        material={nodes.Box078.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object268.geometry}
        material={nodes.Object268.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane217.geometry}
        material={nodes.Plane217.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object269.geometry}
        material={nodes.Object269.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane218.geometry}
        material={nodes.Plane218.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object270.geometry}
        material={nodes.Object270.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane219.geometry}
        material={nodes.Plane219.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object271.geometry}
        material={nodes.Object271.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane220.geometry}
        material={nodes.Plane220.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object272.geometry}
        material={nodes.Object272.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane221.geometry}
        material={nodes.Plane221.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object273.geometry}
        material={nodes.Object273.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane222.geometry}
        material={nodes.Plane222.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object274.geometry}
        material={nodes.Object274.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane223.geometry}
        material={nodes.Plane223.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object275.geometry}
        material={nodes.Object275.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane224.geometry}
        material={nodes.Plane224.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object276.geometry}
        material={nodes.Object276.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane225.geometry}
        material={nodes.Plane225.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object277.geometry}
        material={nodes.Object277.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane226.geometry}
        material={nodes.Plane226.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object278.geometry}
        material={nodes.Object278.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane227.geometry}
        material={nodes.Plane227.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object279.geometry}
        material={nodes.Object279.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane228.geometry}
        material={nodes.Plane228.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box081.geometry}
        material={nodes.Box081.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder227.geometry}
        material={nodes.Cylinder227.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder228.geometry}
        material={nodes.Cylinder228.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder229.geometry}
        material={nodes.Cylinder229.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder230.geometry}
        material={nodes.Cylinder230.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder231.geometry}
        material={nodes.Cylinder231.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder232.geometry}
        material={nodes.Cylinder232.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder233.geometry}
        material={nodes.Cylinder233.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder234.geometry}
        material={nodes.Cylinder234.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder235.geometry}
        material={nodes.Cylinder235.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder236.geometry}
        material={nodes.Cylinder236.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder237.geometry}
        material={nodes.Cylinder237.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder238.geometry}
        material={nodes.Cylinder238.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder239.geometry}
        material={nodes.Cylinder239.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder240.geometry}
        material={nodes.Cylinder240.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder241.geometry}
        material={nodes.Cylinder241.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder242.geometry}
        material={nodes.Cylinder242.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder243.geometry}
        material={nodes.Cylinder243.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder244.geometry}
        material={nodes.Cylinder244.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder245.geometry}
        material={nodes.Cylinder245.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder246.geometry}
        material={nodes.Cylinder246.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder247.geometry}
        material={nodes.Cylinder247.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder248.geometry}
        material={nodes.Cylinder248.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder249.geometry}
        material={nodes.Cylinder249.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder250.geometry}
        material={nodes.Cylinder250.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder251.geometry}
        material={nodes.Cylinder251.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder252.geometry}
        material={nodes.Cylinder252.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Lag_010.geometry}
        material={nodes.Lag_010.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object280.geometry}
        material={nodes.Object280.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object281.geometry}
        material={nodes.Object281.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object282.geometry}
        material={nodes.Object282.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane229.geometry}
        material={nodes.Plane229.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane230.geometry}
        material={nodes.Plane230.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube056.geometry}
        material={nodes.Tube056.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve085.geometry}
        material={nodes.Curve085.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve086.geometry}
        material={nodes.Curve086.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve087.geometry}
        material={nodes.Curve087.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve088.geometry}
        material={nodes.Curve088.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve089.geometry}
        material={nodes.Curve089.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve090.geometry}
        material={nodes.Curve090.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve091.geometry}
        material={nodes.Curve091.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve092.geometry}
        material={nodes.Curve092.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve093.geometry}
        material={nodes.Curve093.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve094.geometry}
        material={nodes.Curve094.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane232.geometry}
        material={nodes.Plane232.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane233.geometry}
        material={nodes.Plane233.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object284.geometry}
        material={nodes.Object284.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder226.geometry}
        material={nodes.Cylinder226.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve095.geometry}
        material={nodes.Curve095.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve096.geometry}
        material={nodes.Curve096.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder224.geometry}
        material={nodes.Cylinder224.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder225.geometry}
        material={nodes.Cylinder225.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube058.geometry}
        material={nodes.Tube058.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube059.geometry}
        material={nodes.Tube059.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube060.geometry}
        material={nodes.Tube060.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube061.geometry}
        material={nodes.Tube061.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane214.geometry}
        material={nodes.Plane214.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box073.geometry}
        material={nodes.Box073.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box074.geometry}
        material={nodes.Box074.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube051.geometry}
        material={nodes.Tube051.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder193.geometry}
        material={nodes.Cylinder193.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object266.geometry}
        material={nodes.Object266.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box070.geometry}
        material={nodes.Box070.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box071.geometry}
        material={nodes.Box071.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box072.geometry}
        material={nodes.Box072.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object251.geometry}
        material={nodes.Object251.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane200.geometry}
        material={nodes.Plane200.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object252.geometry}
        material={nodes.Object252.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane201.geometry}
        material={nodes.Plane201.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object253.geometry}
        material={nodes.Object253.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane202.geometry}
        material={nodes.Plane202.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object254.geometry}
        material={nodes.Object254.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane203.geometry}
        material={nodes.Plane203.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object255.geometry}
        material={nodes.Object255.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane204.geometry}
        material={nodes.Plane204.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object256.geometry}
        material={nodes.Object256.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane205.geometry}
        material={nodes.Plane205.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object257.geometry}
        material={nodes.Object257.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane206.geometry}
        material={nodes.Plane206.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object258.geometry}
        material={nodes.Object258.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane207.geometry}
        material={nodes.Plane207.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object259.geometry}
        material={nodes.Object259.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane208.geometry}
        material={nodes.Plane208.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object260.geometry}
        material={nodes.Object260.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane209.geometry}
        material={nodes.Plane209.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object261.geometry}
        material={nodes.Object261.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane210.geometry}
        material={nodes.Plane210.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object262.geometry}
        material={nodes.Object262.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane211.geometry}
        material={nodes.Plane211.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box075.geometry}
        material={nodes.Box075.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder197.geometry}
        material={nodes.Cylinder197.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder198.geometry}
        material={nodes.Cylinder198.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder199.geometry}
        material={nodes.Cylinder199.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder200.geometry}
        material={nodes.Cylinder200.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder201.geometry}
        material={nodes.Cylinder201.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder202.geometry}
        material={nodes.Cylinder202.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder203.geometry}
        material={nodes.Cylinder203.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder204.geometry}
        material={nodes.Cylinder204.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder205.geometry}
        material={nodes.Cylinder205.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder206.geometry}
        material={nodes.Cylinder206.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder207.geometry}
        material={nodes.Cylinder207.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder208.geometry}
        material={nodes.Cylinder208.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder209.geometry}
        material={nodes.Cylinder209.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder210.geometry}
        material={nodes.Cylinder210.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder211.geometry}
        material={nodes.Cylinder211.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder212.geometry}
        material={nodes.Cylinder212.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder213.geometry}
        material={nodes.Cylinder213.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder214.geometry}
        material={nodes.Cylinder214.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder215.geometry}
        material={nodes.Cylinder215.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder216.geometry}
        material={nodes.Cylinder216.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder217.geometry}
        material={nodes.Cylinder217.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder218.geometry}
        material={nodes.Cylinder218.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder219.geometry}
        material={nodes.Cylinder219.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder220.geometry}
        material={nodes.Cylinder220.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder221.geometry}
        material={nodes.Cylinder221.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder222.geometry}
        material={nodes.Cylinder222.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Lag_009.geometry}
        material={nodes.Lag_009.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object263.geometry}
        material={nodes.Object263.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object264.geometry}
        material={nodes.Object264.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object265.geometry}
        material={nodes.Object265.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane212.geometry}
        material={nodes.Plane212.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane213.geometry}
        material={nodes.Plane213.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube050.geometry}
        material={nodes.Tube050.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve073.geometry}
        material={nodes.Curve073.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve074.geometry}
        material={nodes.Curve074.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve075.geometry}
        material={nodes.Curve075.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve076.geometry}
        material={nodes.Curve076.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve077.geometry}
        material={nodes.Curve077.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve078.geometry}
        material={nodes.Curve078.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve079.geometry}
        material={nodes.Curve079.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve080.geometry}
        material={nodes.Curve080.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve081.geometry}
        material={nodes.Curve081.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve082.geometry}
        material={nodes.Curve082.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane215.geometry}
        material={nodes.Plane215.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane216.geometry}
        material={nodes.Plane216.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object267.geometry}
        material={nodes.Object267.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder196.geometry}
        material={nodes.Cylinder196.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve083.geometry}
        material={nodes.Curve083.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Curve084.geometry}
        material={nodes.Curve084.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder194.geometry}
        material={nodes.Cylinder194.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder195.geometry}
        material={nodes.Cylinder195.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube052.geometry}
        material={nodes.Tube052.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube053.geometry}
        material={nodes.Tube053.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube054.geometry}
        material={nodes.Tube054.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube055.geometry}
        material={nodes.Tube055.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box023.geometry}
        material={nodes.Box023.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Body001.geometry}
        material={nodes.Body001.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object138.geometry}
        material={nodes.Object138.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane143.geometry}
        material={nodes.Plane143.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object137.geometry}
        material={nodes.Object137.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane142.geometry}
        material={nodes.Plane142.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object136.geometry}
        material={nodes.Object136.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane141.geometry}
        material={nodes.Plane141.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object139.geometry}
        material={nodes.Object139.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane144.geometry}
        material={nodes.Plane144.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object140.geometry}
        material={nodes.Object140.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane145.geometry}
        material={nodes.Plane145.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object141.geometry}
        material={nodes.Object141.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane146.geometry}
        material={nodes.Plane146.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.obj1.geometry}
        material={nodes.obj1.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder003.geometry}
        material={nodes.Cylinder003.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder002.geometry}
        material={nodes.Cylinder002.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Cylinder001.geometry}
        material={nodes.Cylinder001.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Sphere001.geometry}
        material={nodes.Sphere001.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box002.geometry}
        material={nodes.Box002.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube002.geometry}
        material={nodes.Tube002.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object001.geometry}
        material={nodes.Object001.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.TextPlus001.geometry}
        material={materials.wire_255255255}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Tube001.geometry}
        material={nodes.Tube001.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box048.geometry}
        material={nodes.Box048.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box049.geometry}
        material={nodes.Box049.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box050.geometry}
        material={nodes.Box050.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box051.geometry}
        material={nodes.Box051.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box052.geometry}
        material={nodes.Box052.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box053.geometry}
        material={nodes.Box053.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box054.geometry}
        material={nodes.Box054.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box055.geometry}
        material={nodes.Box055.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box056.geometry}
        material={nodes.Box056.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Box057.geometry}
        material={nodes.Box057.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object210.geometry}
        material={nodes.Object210.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Plane165.geometry}
        material={nodes.Plane165.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object211.geometry}
        material={nodes.Object211.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object212.geometry}
        material={nodes.Object212.material}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
      <mesh
        geometry={nodes.Object216.geometry}
        material={materials.wire_008061138}
        position={[1, 0.26, -0.87]}
        rotation={[Math.PI / 2, 0, 0.09]}
        scale={0.12}
      />
    </group>
  );
}

useGLTF.preload("/cyberpunk-robot.glb");
